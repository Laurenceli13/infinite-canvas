...
import { ImagePlus, MapPinned, Package, Trash2, Users, WandSparkles } from "lucide-react";
...
export function CanvasVideoWorkflowForm({ config, running, onRunningChange, onClose, onRun }: Props) {
    ...
    const resolutionOptions = VIDEO_RESOLUTIONS.filter((item) => configuredResolutions.includes(item.value) && (!seedance || item.value !== "4k") && (!fastSeedance || item.value !== "1080p"));
    ...
    const resolutionSignature = resolutionOptions.map((item) => item.requestValue).join(",");
    useEffect(() => {
        if (!resolutionOptions.length) return;
        const current = normalizedVideoResolution(videoResolution);
        const matched = resolutionOptions.find((item) => item.value === current);
        if (!matched) setVideoResolution(resolutionOptions[0].requestValue);
    ...
    const nineViewCredits = imageModel
        ? requestCreditCost({ channelMode: config.channelMode, modelCosts: config.modelCosts, modelPricingRules: config.modelPricingRules, model: imageModel, capability: "image", count: characterFiles.length, quality: config.quality, size: config.size })
        : 0;
    const frameCredits = imageModel
        ? requestCreditCost({ channelMode: config.channelMode, modelCosts: config.modelCosts, modelPricingRules: config.modelPricingRules, model: imageModel, capability: "image", count: 2, quality: config.quality, size: frameSize })
        : 0;
    const videoCredits = syncGenerateVideo && videoModel
    ...
    const optimizeScript = async () => {
        ...
        try {
            ...
            const answer = await requestImageQuestion(
                ...
                [{ role: "user", content: buildVideoScriptOptimizationPrompt(script, extraRequirements) }],
                (text) => {
                    streamed = text;
                },
            );
    ...
    const run = async () => {
        ...
        if (syncGenerateVideo && !resolutionOptions.length) {
    ...
    return (
        <div className="space-y-5">
            <Alert
                ...
                description={syncGenerateVideo ? "每张角色图代表一个独立角色。制作参考总览会合并全部角色、物品与场景，避免供应商参考图数量上限造成素材丢失。" : "关闭同步生成视频后不要求选择视频模型，也不会扣除视频积分。请验收九视图和首尾帧后，在画布的“最终视频”配置节点选择模型并点击生成。"}
            />
            ...
            <section className="grid gap-3 md:grid-cols-2">
                <label className="block">
                    <span className="mb-1.5 flex items-center justify-between gap-3 text-sm font-medium text-stone-800 dark:text-stone-200">
                        <span>视频文案 <span className="text-red-500">*</span></span>
                ...
                <label className="block">
                    ...
                    <Input.TextArea value={extraRequirements} rows={7} maxLength={4000} showCount disabled={running} placeholder="必须保留的服装、物品使用方式、镜头禁区、动作节奏、品牌规范等..." onChange={(event) => setExtraRequirements(event.target.value)} />
                </label>
            </section>
            ...
            <section className="grid gap-3 md:grid-cols-4">
                <label className="block">
                    ...
                    <InputNumber className="w-full" min={minSeconds} max={maxSeconds} value={Number(videoSeconds)} disabled={running || !syncGenerateVideo} addonAfter="秒" onChange={(value) => setVideoSeconds(String(Math.max(minSeconds, Math.min(maxSeconds, Number(value || minSeconds)))))} />
                </label>
                <label className="block">
                    <span className="mb-1.5 block text-sm font-medium text-stone-800 dark:text-stone-200">清晰度</span>
                    <Select className="w-full" value={videoResolution || undefined} options={resolutionOptions.map((item) => ({ value: item.requestValue, label: item.label }))} disabled={running || !syncGenerateVideo || !resolutionOptions.length} onChange={setVideoResolution} />
                </label>
                <label className="block">
            ...
            <section className="flex flex-wrap items-center justify-between gap-3 border-y border-stone-200 py-3 dark:border-stone-800">
                <div className="text-xs text-stone-500">
                    九视图 {nineViewCredits.toFixed(2)} + 首尾帧 {frameCredits.toFixed(2)}{syncGenerateVideo ? ` + 视频 ${videoCredits.toFixed(2)}` : "（视频暂不生成）"}
                </div>
            ...
            <footer className="flex items-center justify-end gap-2 pt-1">
                <Button disabled={running || optimizing} onClick={onClose}>取消</Button>
...
function UploadBucket({ inputRef, label, hint, required, files, max, disabled, icon, onFiles, onRemove }: UploadBucketProps) {
    return (
        <div className="min-w-0">
            ...
            >
                <span className="inline-flex items-center gap-2 font-medium">{icon}{label}{required ? <span className="text-red-500">*</span> : null}</span>
                <span className="text-xs text-stone-500">{hint}</span>
                <span className="text-xs tabular-nums">{files.length}/{max}</span>
            </button>
            {files.length ? (
                <div className="mt-2 flex max-h-20 flex-wrap gap-1 overflow-y-auto">
                    {files.map((file, index) => (
                        <Tag key={`${file.name}-${file.lastModified}-${index}`} className="max-w-full" closable={!disabled} closeIcon={<Trash2 className="size-3" />} onClose={() => onRemove(index)}>
                            <span className="inline-block max-w-32 truncate align-bottom">{file.name}</span>
                        </Tag>
                    ))}
                </div>
            ) : null}
